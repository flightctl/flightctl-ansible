...
class FlightctlModule(AnsibleModule):
    AUTH_ARGSPEC = dict(
        flightctl_host=dict(required=False, fallback=(env_fallback, ['FLIGHTCTL_HOST'])),
        flightctl_username=dict(required=False, fallback=(env_fallback, ['FLIGHTCTL_USERNAME'])),
        flightctl_password=dict(no_log=True, required=False, fallback=(env_fallback, ['FLIGHTCTL_PASSWORD'])),
        flightctl_validate_certs=dict(type='bool', aliases=['verify_ssl'], default=False, fallback=(env_fallback, ['FLIGHTCTL_VERIFY_SSL'])),
        flightctl_request_timeout=dict(type='float', required=False, fallback=(env_fallback, ['FLIGHTCTL_REQUEST_TIMEOUT'])),
        flightctl_token=dict(
            type='str', no_log=True, required=False, fallback=(env_fallback, ['FLIGHTCTL_TOKEN'])
        ),
        flightctl_config_file=dict(required=False, type='path', aliases=['config_file']),
    )
    short_params = {
        'host': 'flightctl_host',
    ...
    def __init__(self, argument_spec: Optional[Any] = None, error_callback: Optional[Any] = None, warn_callback: Optional[Any] = None, **kwargs: Any):
        full_argspec = {}
    ...
    def ensure_host_url(self) -> None:
        ...
        try:
            ...
            raise FlightctlException(f"Unable to parse flightctl_host as a URL ({e}): {self.host}") from e
