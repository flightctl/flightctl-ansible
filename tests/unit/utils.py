...
def find_working_profile():
    ...
    try:
        ...
        available_profiles = []
        for _, name, ispkg in pkgutil.iter_modules(profiles_path):
    ...
    for profile_name in test_profiles:
        try:
            from ansible.module_utils._internal import _json
            # Try to get the serialization profile name transformation
            transformed_name = _json.get_module_serialization_profile_name(
                profile_name, True  # controller_to_module=True
            )
